...
def extract_mesh(
    ...
    while cur_depth <= max_depth:
        ...
        octree.adaptive_subdivide(centroid_udfs, centroid_grads, 0.002) #original 0.002
    ...
    v, vi, vj, wrapper = igl.remove_duplicate_vertices(np.array(octree.mesh_v), tri_faces, 1e-7)
    f = vj[tri_faces]
    ...
    return v, f, query_time, optim_time, mesh_time


def extract_mesh_mod(
        udf_func,
        udf_grad_func,
        batch_size = 150000,
        max_depth=7
    ...
    while cur_depth <= max_depth:
        ...
        # adaptively subdivide the cells
        octree.adaptive_subdivide(centroid_udfs, centroid_grads, 0.02)
    ...
    indices, projections = octree.get_projections_for_checking_validity()
    projection_udfs = query_udf(udf_func, projections, batch_size)
    octree.set_grid_validity(indices, projection_udfs < 0.02)

    query_time = time.time() - start
    start = time.time()

    # (reliable udf threshold, corner factor, edge factor, face factor, singular value threshold)
    octree.batch_solve_mod(0.001, 1.0, 1.0, 0.15, 0.08)
