...
# def marching_cubes(float[:, :, :] im not None,
#                    LutProvider luts, int st=1, int classic=0,
#                    np.ndarray[np.npy_bool, ndim=3, cast=True] mask=None):
#     """ marching_cubes(im, double isovalue, LutProvider luts, int st=1, int classic=0)
#     Main entry to apply marching cubes.
...
#     cdef float num_already_computed
#     cdef float num_agreeing
...
def marching_cubes_pseudosdf(float[:, :, :, :] im not None,
                   LutProvider luts, int st=1, int classic=0,
                   np.ndarray[np.npy_bool, ndim=3, cast=True] mask=None):
    """ marching_cubes(im, double isovalue, LutProvider luts, int st=1, int classic=0)
    Main entry to apply marching cubes.
    ...
    # Create cell to use throughout
    cdef Cell cell = Cell(luts, Nx+1, Ny+1, Nz+1)
    ...
    assert st > 0
    cdef int Nx_bound, Ny_bound, Nz_bound
    Nx_bound, Ny_bound, Nz_bound = Nx-1*st, Ny-1*st, Nz-1*st  # precalculated index range
    cdef float[:] base_vec
